...
const getStripeProvisioning = async (organizationId: string | null | undefined) => {
	if (!organizationId) return { allowedMembers: 0, currentMembers: 0 };
	...
	return { allowedMembers: Number(product.metadata.allowedMembers), currentMembers };
};
